...
func (w *worker) commitTransaction(tx *types.Transaction, coinbase common.Address) ([]*types.Log, error) {
	...
	log.Info("TxLifeCycleTest: Ready to apply single transaction (miner->worker.go->commitTransaction)", "txHash", tx.Hash())
	...
	log.Info("TxLifeCycleTest: Applied transaction and added to worker's txs and receipts (miner->worker.go->commitTransaction)")
...
func (w *worker) commitTransactions(txs *types.TransactionsByPriceAndNonce, coinbase common.Address, interrupt *int32) bool {
	...
	for {
		...
		log.Info("TxLifeCycleTest: Preparing Tx (miner->worker.go->commitTransactions)", "TxHash", tx.Hash())
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	if len(localTxs) > 0 {
		...
		log.Info("TxLifeCycleTest: Committing Local transactions, (miner->worker.go->commitNewWork)")
	...
	if len(remoteTxs) > 0 {
		...
		log.Info("TxLifeCycleTest: Committing Remote transactions, (miner->worker.go->commitNewWork)")
		if w.commitTransactions(txs, w.coinbase, interrupt) {
			return
		}
	}
...
func (w *worker) commit(uncles []*types.Header, interval func(), update bool, start time.Time) error {
	...
	if w.isRunning() {
		...
		case w.taskCh <- &task{receipts: receipts, state: s, block: block, createdAt: time.Now()}:
			...
			log.Info("TxLifeCycleTest: Post transactions state commit, (miner->worker.go->commit)")
